...
def computeSNR(inputFile, window, M, N, H):
    ...
    fs, x = UF.wavread(wav_file)

    fftbins = M % 2 == 0        # true if M is even.
    w = get_window(window, M, fftbins)

    y = stft(x, w, N, H)

    # compute SNR1 ie over full length.
    E_x = np.sum(x ** 2)
    noise = y - x
    E_noise = np.sum(noise ** 2)
    snr1 = 10 * math.log10(E_x / E_noise + eps)

    # compute SNR2 ie after removing M (window_size) from start and end.
    E_x = np.sum(x[M:-M] ** 2)
    noise = y[M:-M] - x[M:-M]
    E_noise = np.sum(noise ** 2)
    snr2 = 10 * math.log10(E_x / E_noise + eps)
    print("SNR1, SNR2 = ", (snr1, snr2))

    # verify synthesised audio
#     UF.wavwrite(y, fs, "synth.wav")
#     UF.wavplay("synth.wav")
    return snr1, snr2

# (67.57748352378475, 304.68394693221649).
file_path = r"D:\Users\Charley\Documents\Esperanza\repos\sms-tools\sounds\piano.wav"
computeSNR(file_path, "blackman", M=513, N=2048, H=128)

# (89.510506656299285, 306.18696700251388)
file_path = r"D:\Users\Charley\Documents\Esperanza\repos\sms-tools\sounds\sax-phrase-short.wav"
computeSNR(file_path, "hamming", M=512, N=1024, H=64)

# (74.631476225366825, 304.26918192997738).
file_path = r"D:\Users\Charley\Documents\Esperanza\repos\sms-tools\sounds\rain.wav"
computeSNR(file_path, "hann", M=1024, N=2048, H=128)