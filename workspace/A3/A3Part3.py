...
def testRealEven(x):
    ...
    # get zero-phase windowed version of signal (without zero-padding)
    M = len(x)
    ...
    dftbuffer[:hM1] = x[hM2:]
    dftbuffer[-hM2:] = x[:hM2]
    print("DFT Buffer is: ", dftbuffer)

    # compute dft of the buffer
    X = fft(dftbuffer, n=M)
    print("FFT is: ", X)

    # test spectrum for real and even
    pass_mag_test = True
    abs_X = abs(X)
    for i in np.arange(1,M):
        print("Index: ", i)
        if abs_X[i] != abs_X[M-i]:
            pass_mag_test = False
            print('Test for even magnitude spectrum failed: {} != {}'.format(abs_X[i], abs_X[M-1-i]))
            break
        
        print('Test for even magnitude spectrum passed!')
    pass_imag_test = np.sum(X.imag < 1e-6) == len(X)
    print("Imaginary test returned: ", pass_imag_test)

    is_real_even = False
    if pass_mag_test and pass_imag_test:
        is_real_even = True
