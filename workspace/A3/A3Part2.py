...
def optimalZeropad(x, fs, f):
    ...
    print("Length of x={}; fs={}; f={}".format(len_x, fs, f))

    # Calculate the no of samples needed to make the length of the signal an integer multiple 
    # of the samples per period.
    zero_pad_len = len_x % period_in_samples
    if zero_pad_len > 0: 
        zero_pad_len = period_in_samples - zero_pad_len
    print('Length of required zero-padding is: ', zero_pad_len)

    # perform zero padding
    x_padded = np.pad(x, pad_width=(0, int(zero_pad_len)), mode='constant', constant_values=0)
    M = len(x_padded)
    print("Length of padded signal is: ", M)

    # compute fft, extract first half +1, and convert mag spectrum to db.
    X = fft(x_padded, n=M)
    X = X[:M//2 + 1]
...
fs, f1 = 10000, 250
t_array = np.arange(0, 210.0/fs, 1.0/fs)
x = np.cos(2 * np.pi * f1 * t_array)
