...
fs, f1 = 1000, 110
t_array = np.arange(0, 512.0/fs, 1.0/fs)
x = np.cos(2 * np.pi * f1 * t_array)
print('x is of shape: {}'.format(x.shape))
mX1_80, mX2_80, mX3_80 = zpFFTsizeExpt(x, fs)

k1 = fs * np.arange(len(mX1_80)) / 256
k2 = fs * np.arange(len(mX2_80)) / 512
k3 = fs * np.arange(len(mX3_80)) / 512

plt.figure()
plt.plot(k1, mX1_80, label='x=256; w=256; N=256')
plt.plot(k2, mX2_80, label='x=512; w=512; N=512')
plt.plot(k3, mX3_80, label='x=256; w=256; N=512')
plt.legend()
plt.show()